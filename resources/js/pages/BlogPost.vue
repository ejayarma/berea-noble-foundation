...
<template>
    <WebsiteLayout class="min-h-svh">
        ...
        <section class="relative h-[500px] overflow-hidden">
            ...
            <div class="absolute bottom-0 left-0 right-0 px-4 py-12">
                <div class="max-w-4xl mx-auto">
                    <span
                        :class="getCategoryColor(post.category)"
                        class="inline-block px-4 py-1.5 text-white text-sm font-medium rounded-full mb-4"
                    >
                        {{ post.category }}
                    </span>
                    ...
                    <div class="flex items-center gap-6 text-white/90 text-sm">
                        <div class="flex items-center gap-2">
                            <div v-if="post.author.avatar" class="w-10 h-10 rounded-full overflow-hidden">
                                <img :src="post.author.avatar" :alt="post.author.name" class="w-full h-full object-cover" />
                            </div>
                            <div v-else class="w-10 h-10 rounded-full bg-orange-500 flex items-center justify-center text-white font-semibold">
        ...
        <!-- Content Section -->
        <article class="py-16 lg:px-10">
            <div class="container-primary">
                <div class="prose prose-lg max-w-none prose-headings:font-bold prose-headings:text-gray-900 prose-p:text-gray-700 prose-a:text-orange-500 hover:prose-a:text-orange-600 prose-img:rounded-lg prose-img:shadow-lg" v-html="post.content" />
            </div>
        </article>
        ...
        <section class="max-w-4xl mx-auto px-4 py-8 border-t border-gray-200">
            <div class="flex items-center justify-between flex-wrap gap-4">
                <h3 class="text-lg font-semibold text-gray-900">Share this story</h3>
                <div class="flex gap-3">
                <a
                    :href="`https://twitter.com/intent/tweet?url=${encodeURIComponent(page.url)}&text=${encodeURIComponent(post.title)}`"
                    target="_blank"
                    rel="noopener noreferrer"
                    class="w-10 h-10 flex items-center justify-center rounded-full bg-gray-100 hover:bg-orange-500 hover:text-white transition-colors"
                >
        ...
        <!-- Related Posts -->
        <section v-if="relatedPosts && relatedPosts.length > 0" class="bg-gray-50 py-16">
            <div class="max-w-7xl mx-auto px-4">
                ...
                >
                    ...
                    <span
                        ...
                        class="absolute top-3 left-3 z-10 px-3 py-1 text-white text-xs font-medium rounded-full"
                    >
                    ...
                    <div class="p-5">
                    <p class="text-sm text-gray-500 mb-2">{{ formatDate(relatedPost.published_at) }}</p>
                    <h3 class="text-lg font-semibold text-gray-900 group-hover:text-orange-500 transition-colors line-clamp-2">
                        {{ relatedPost.title }}
                    </h3>
                    </div>
                </Link>
